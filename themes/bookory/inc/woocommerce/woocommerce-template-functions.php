...
if (!function_exists('bookory_products_per_page_select')) {
    ...
    function bookory_products_per_page_select() {
        ...
        $max_col      = apply_filters('bookory_products_row_step_max', 24);
        $array_option = [];
        if ($max_col > 6) {
            for ($i = 3; $i <= $max_col; $i++) {
                $array_option[] = $row * $i;
        ...
        $current_variation = isset($_GET['per_page']) ? $_GET['per_page'] : 24;
        ?>
...
if (isset($_GET['per_page'])) {
    add_filter('loop_shop_per_page', 'bookory_loop_shop_per_page', 24);
}

function bookory_loop_shop_per_page($cols) {
    $cols = isset($_GET['per_page']) ? $_GET['per_page'] : 24;
    return $cols;
}
