...
class Bookory_Megamenu_Walker extends Walker_Nav_Menu {
    ...
    public function display_element($element, &$children_elements, $max_depth, $depth, $args, &$output) {
        ...
        if ($this->is_megamenu($element, $depth) && $depth == 0) {
            ...
            $this->remove_display_element($element, $children_elements, $element->$id_field);
        }
        parent::display_element($element, $children_elements, $max_depth, $depth, $args, $output);
    }

    public function remove_display_element($element, &$children_elements, $id) {
        if (!$element) {
            return;
        }
        if (isset($children_elements[$id]) && is_array($children_elements[$id])) {
            foreach ($children_elements[$id] as $_item) {
                $this->remove_display_element($element, $children_elements, $_item->ID);
