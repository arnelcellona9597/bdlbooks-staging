...
class WCPA_Order_Meta {
	...
	public function order_meta_plain( $v, $show_price = true, $quantity = 1, $product = false ) {
		...
		if ( $v['type'] == 'file' ) {
			...
			if ( $v['price'] && $show_price ) {
				return implode( "\r\n", array_map( function ( $pro, $default_price, $quanti ) use ( $field_price_multiplier, $product ) {	
					
					$quantity = (isset($quanti) && !empty($quanti)) ? $quanti : 1;
					if($pro){
						
						$pro_link = $pro->get_permalink();

						if ( $default_price ) {
							if ( $product ) {
								$price = wcpa_get_price_cart( $product, $default_price );
							} else {
								$price = $default_price;
							}

							return $pro->get_title() .' | '. 'x '.$quantity . ' | ' . $pro_link . ' | (' . wcpa_price( $price * $field_price_multiplier, 1 ) . ')';
						} else {
							return $pro->get_title() .' | '. 'x '.$quantity . ' | ' . $pro_link;
						}
					}
				...
				return implode( "\r\n", array_map( function ( $prod ) {
					if($prod){
						...
						$pro_link = $prod->get_permalink();

						return $prod->get_title() . ' | ' . $pro_link;
					}
				}, $v['value'] ) );
			}
	...
	public function display_meta_value_image( $value ) {
		...
		foreach ( $value['value'] as $k => $v ) {
			...
			if ( isset( $v['image'] ) && $v['image'] !== false ) {
				...
				$display .= '<p class="wcpa_image">';
				$display .= __( 'Other:', 'wcpa-text-domain' ) . ' ' . $v['value'];
			}
									...
									if($meta_data[ $k ]['value']){
										foreach($meta_data[ $k ]['value'] as $key => $f_val){
											if($f_val){
												...
												if($f_val['url'] !== trim( $data[ $k ][$key] )){
													$f_val['url']       = trim( sanitize_text_field( $data[ $k ][ $key ] ) );
													$f_val['path']      = false;
													$f_val['file_name'] = wp_basename( $data[ $k ][ $key ] );
													$file_type                             = wp_check_filetype( $data[ $k ][ $key ] );
													$meta_value_temp['value'][ $key ]    = $f_val;
													$meta_value_temp['type'][ $key ]     = $v['type'];
