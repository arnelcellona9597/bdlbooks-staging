...
if(!function_exists('wcpa_get_validation_message')){
    ...
    function wcpa_get_validation_message($field, $v, $replace = '') {
        ...
        $db_message     = isset($v_st['validation_'.$field]) ? __($v_st['validation_'.$field], 'wcpa-text-domain') : '';
        $field_message  = isset($v->$field) ? __($v->$field, 'wcpa-text-domain') : '';

        if($replace){
            $defaultMessage = str_replace( '%s', $replace, 
                ( isset($db_message) && !empty($db_message) ) 
                ? $db_message : $default_replacer[$field]['message']);
            return ($field_message != '') ? str_replace( $default_replacer[$field]['replace'], $replace, $field_message ) : $defaultMessage;
        } else {
            $defaultMessage = (isset($db_message) && !empty($db_message) ) 
                ? $db_message : $default_replacer[$field]['message'];
            return ($field_message != '') ? str_replace($default_replacer[$field]['replace'], '%s', $field_message) : $defaultMessage;
        }
    }
}
...
if (!function_exists('wcpa_check_woocommerce_active')) {
    /**
     * Check whether woocommerce is active
     * @return boolean
     */
    function wcpa_check_woocommerce_active(){
