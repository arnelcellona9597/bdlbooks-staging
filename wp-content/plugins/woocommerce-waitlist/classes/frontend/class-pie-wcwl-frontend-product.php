	...
	abstract class Pie_WCWL_Frontend_Product {
		...
		protected function get_product_id() {
			...
			if ( Pie_WCWL_Frontend_Init::is_ajax_variation_request() ) {
				...
				if ( $this->has_wpml ) {
					$product_id = isset( $_REQUEST['product_id'] ) ? Pie_WCWL_Frontend_Init::get_main_product_id( absint( $_REQUEST['product_id'] ) ) : 0;
				} else {
		...
		protected function new_user_can_join_waitlist() {
			if ( 'yes' == get_option( 'woocommerce_waitlist_registration_needed' ) ) {
				/**
				 * Filter notification text when users must register to use waitlist
				 * 
				 * @since 2.4.0
				 */
				...
				/**
				 * Filter notification text when invalid email given
				 * 
				 * @since 2.4.0
				 */
				...
				/**
				 * Filter notification text when no products selected for grouped product
				 * 
				 * @since 2.4.0
				 */
		...
		protected function handle_waitlist_when_new_user( $waitlist ) {
			if ( $this->new_user_can_join_waitlist() ) {
				...
				/**
				 * Filter notification text when user joins waitlist
				 * 
				 * @since 2.4.0
				 */
		...
		protected function toggle_waitlist_action( $waitlist ) {
			$action = isset( $_GET[ WCWL_SLUG . '_action' ] ) ? sanitize_text_field( $_GET[ WCWL_SLUG . '_action' ] ) : '';
			if ( ! $action ) {
				return;
			}
			if ( 'leave' == $action && $waitlist->user_is_registered( $this->user->ID ) && $waitlist->unregister_user( $this->user ) ) {
				/**
				 * Filter notification text when user leaves waitlist
				 * 
				 * @since 2.4.0
				 */
				wc_add_notice( apply_filters( 'wcwl_leave_waitlist_success_message_text', $this->leave_waitlist_success_message_text ) );
			}
			if ( 'join' === $action && ! $waitlist->user_is_registered( $this->user->ID ) && $waitlist->register_user( $this->user ) ) {
				/**
				 * Filter notification text when user joins waitlist
				 * 
				 * @since 2.4.0
		...
		protected function get_waitlist_email_field() {
			...
			/**
			 * Filter label for waitlist email field
			 * 
			 * @since 2.4.0
			 */
			$label = apply_filters( 'wcwl_email_field_label', $this->email_field_placeholder_text );
			/**
			 * Filter HTML for waitlist email field
			 * 
			 * @since 2.4.0
			 */
			$html .= apply_filters( 'wcwl_frontend_email_field_html', '<div class="wcwl_email_field">
						<label for="wcwl_email" class="wcwl_email_label wcwl_hide">' . $label . '</label>
			...
			if ( 'yes' == get_option( 'woocommerce_waitlist_new_user_opt-in' ) ) {
				/**
				 * Filter explanation text for waitlist opt-in
				 * 
				 * @since 2.4.0
				 */
			...
			/**
			 * Filter HTML for waitlist email field for logged out users
			 * 
			 * @since 2.4.0
			 */
		...
		protected function get_waitlist_opt_in_html( $notice ) {
			...
			/**
			 * Filter HTML for waitlist opt-in
			 * 
			 * @since 2.4.0
			 */
		...
		public function create_button_url( $action, $product_id ) {
			global $wp;
			if ( ( ! isset( $wp->request ) || empty( $wp->request ) ) && ( isset( $_SERVER['REQUEST_URI']) && '/?wc-ajax=get_variation' == $_SERVER['REQUEST_URI'] ) ) {
				$referer     = isset( $_SERVER['HTTP_REFERER'] ) ? esc_url_raw( $_SERVER['HTTP_REFERER'] ) : '';
				$request_url = add_query_arg( array(), $referer );
			} else {
			...
			/**
			 * Filter URL for waitlist button
			 * 
			 * @since 2.4.0
			 */
		...
		protected function setup_text_strings() {
			...
			/* translators: %1$s opening <a> tag links to my account page, %2$s closing <a> tag */
