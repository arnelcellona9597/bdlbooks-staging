...
if (!class_exists('WC_SA_Order')) :
    ...
    class WC_SA_Order
    {
        ...
        public function __construct()
        {
            ...
            add_filter('woocommerce_valid_order_statuses_for_payment_complete', array($this, 'valid_order_statuses_for_payment_complete'), 99, 2);
            add_filter('woocommerce_payment_complete_order_status', array($this, 'payment_complete_order_status'), 777, 3);

            // The woocommerce_payment_complete_order_status filter does not work with BACS, COD and Cheque payment methods,
            // so we need to use the *_process_payment_order_status filters.
            add_filter('woocommerce_bacs_process_payment_order_status', array($this, 'process_payment_order_status'), 777, 2);
            add_filter('woocommerce_cod_process_payment_order_status', array($this, 'process_payment_order_status'), 777, 2);
            add_filter('woocommerce_cheque_process_payment_order_status', array($this, 'process_payment_order_status'), 777, 2);
                ...
                foreach ($statuses as $st_id => $value) {
                    ...
                    if (in_array($o_st, $display_for)) {
                        ...
                        $new_actions[$status->label] = array(
                            'url' => wp_nonce_url(admin_url('admin-ajax.php?action=wc_sa_mark_order_status&status=' . $status->label . '&order_id=' . $the_order->get_id()), 'wc-sa-mark-order-status'),
                            'name' => $status->title,
                            'action' => implode(' ', $_a)
                        );
                    }
                            ...
                            foreach ($status->show_action_buttons as $st_key) {
                                ...
                                if (isset($order_statuses[$st_key])) {
                                    ...
                                    $new_actions[$_action] = array(
                                        'url' => wp_nonce_url(admin_url('admin-ajax.php?action=wc_sa_mark_order_status&status=' . $_key . '&order_id=' . $the_order->get_id()), 'woocommerce-mark-order-status'),
                                        'name' => $name,
                                        'action' => $_action
                                    );
                                }
            ...
            <script type="text/html" id="tmpl-wc_as_note_prompt-modal">
                ...
                <form class="wc_as_note_prompt_form" method="post">
                    <div class="media-frame-content" data-columns="10">
                        <?php _e('Add a note for your reference or a note to the customer.', 'woocommerce_status_actions'); ?>
         ...
         * @return string
         */
        public function payment_complete_order_status($new_order_status, $id, $order)
        {
            $saved_st = get_option('wc_sa_payment_gateway_statuses');
            $pm_id = $order->get_payment_method();

            if (!empty($pm_id) && isset($saved_st[$pm_id])) {
                $new_st = $saved_st[$pm_id];
                if (!empty($new_st)) {
                    $new_order_status = str_replace('wc-', '', $new_st);
                }
            }
         ...
         * @return string
         */
        public function process_payment_order_status($new_order_status, $order)
        {
            $saved_st = get_option('wc_sa_payment_gateway_statuses');
            $pm_id = $order->get_payment_method();
        ...
        {
            $_statuses = wc_sa_get_statuses();
            foreach ($_statuses as $_status){
                $statuses[] = $_status->label;
            }
