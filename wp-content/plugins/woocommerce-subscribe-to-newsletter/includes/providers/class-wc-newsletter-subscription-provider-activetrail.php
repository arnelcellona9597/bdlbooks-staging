...
use KoiLab\WC_Newsletter_Subscription\Utilities\Phone_Utils as Phone;
...
class WC_Newsletter_Subscription_Provider_ActiveTrail extends WC_Newsletter_Subscription_Provider {
	...
	public function subscribe( $list, $subscriber ) {
		...
		if ( $subscriber->get_phone() ) {
			$request['sms'] = Phone::format_e164( $subscriber->get_phone(), $subscriber->get_country_code() );
		}

		$response = $this->api_request( 'groups/' . $list . '/members', $request, 'POST' );
