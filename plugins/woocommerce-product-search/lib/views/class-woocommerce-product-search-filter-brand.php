...
class WooCommerce_Product_Search_Filter_Brand extends Filter_Renderer {
	...
	public static function render( $atts = array(), &$results = null ) {
		...
		if ( $taxonomy === false ) {
			...
			if ( $atts['heading'] === null || $atts['heading'] === '' ) {
				if ( !empty( $taxonomy->labels ) && !empty( $taxonomy->labels->singular_name ) ) {
					$atts['heading'] = _x( $taxonomy->labels->singular_name, 'product brand singular name', 'woocommerce-product-search' );
				} else {
					$atts['heading'] = _x( $taxonomy->label, 'product brand label', 'woocommerce-product-search' );
				}
			}
		...
		$query_ixwpst = isset( $_GET['ixwpst'] ) && is_array( $_GET['ixwpst'] ) ? $_GET['ixwpst'] : null;
