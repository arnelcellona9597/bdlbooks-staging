...
class WCPA_Front_End extends WCPA_Order_Meta
{
    ...
    public function __construct($file = '', $version = '1.0.0')
    {
        ...
        if (wcpa_check_woocommerce_active()) {
            ...
            add_filter('wcpay_payment_request_is_product_supported', array($this, 'wcpay_payment_method_in_detail_page'), 10, 2);
            ...
            add_action('in_plugin_update_message-woo-custom-product-addons-pro/start.php',array($this, 'upgrade_version_notice'),10,2);
        }
    }

    public function upgrade_version_notice($plugin_data, $response) {
        if($response && isset($response->new_version) && version_compare('4.999',$response->new_version,'<')){
            echo '<div ><h3 style="color:red">*** Breaking Changes Ahead in new version.
            Please <a href="https://acowebs.com/r/version-pro-5-update" target="_blank">Read</a> Before updating ***</h3></div>';
        }
    }
...
//        }
	    if(!empty($checkout_fields)) {
		    foreach ( $checkout_fields as $fieldset_key => $fieldset ) {
			    if(isset($fieldset['fields'])) {
			    	/** fix to make it compatible with acowebs checkout plugin  */
				    $checkout_key_val_pair['parents'][$fieldset_key] = $fieldset_key;
				    foreach ( $fieldset['fields'] as $field ){
					    if(is_array($field)){
						    foreach($field as $_field){
							    $checkout_key_val_pair['fields'][$fieldset_key][$_field['bindingKey']] = isset($_field['label']) ? $_field['label'] : '';
						    }
					    }

				    }
			    }else  if(!empty($fieldset)) {
				    $checkout_key_val_pair['parents'][$fieldset_key] = $fieldset_key;
				    foreach ( $fieldset as $key => $field ){
					    $checkout_key_val_pair['fields'][$fieldset_key][$key] = isset($field['label']) ? $field['label'] : '';
				    }
    ...
    public function wcpay_payment_method_in_detail_page($status, $product = false)
    {

        if ($product && $this->is_wcpa_product($product->get_id())) {
            $status = false;
        }
    ...
    public function get_fee_label($v, $i = false)
    {
        ...
        if (is_array($v['value']) && $i !== false) {
            ...
            if (isset($v['form_data']->fee_label) && !empty($v['form_data']->fee_label)) {
                ...
                return ($v['label'] == WCPA_EMPTY_LABEL) ? strip_tags(is_array($v['value']) ? '' : $v['value']) : $v['label'];
            }
        }
    ...
    **/
                ...
                foreach ($value['wcpa_combined_products'] as $i => $p) {
                    if (isset($p['value']) && !empty($p['value'])) {
                        foreach ($p['value'] as $k => $v) {
                            $qty = (isset($p['quantities']) && isset($p['quantities'][$k]) && is_numeric($p['quantities'][$k])) ?  $p['quantities'][$k] : 1;
                            // get single product price ie, quantity = 1
                            $current_product[$v->get_id()]['price'] = $p['price'][$k] / $qty;
                        }
                    }
                }
    ...
    public function after_cart_item_name($cart_item, $cart_item_key) {
        if (wcpa_get_option('enable_cart_item_edit', false)) {
            if (
                isset($cart_item[WCPA_CART_ITEM_KEY]) && !empty($cart_item[WCPA_CART_ITEM_KEY]) &&
                isset($cart_item['data']) && !empty($cart_item['data'])
            ) {
                $product        = $cart_item['data'];
                $product_link   = add_query_arg( 'cart_key', $cart_item_key, $product->get_permalink() );
                $edit_text      = __('Edit', 'wcpa-text-domain');
...
//        }
        ...
        if(wcpa_check_woocommerce_active()) {
            ...
            $wcpa_global_vars['cart_url'] = apply_filters( 'woocommerce_add_to_cart_redirect', wc_get_cart_url(), null );
