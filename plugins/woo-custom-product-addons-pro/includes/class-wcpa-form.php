        ...
        if ($this->data) {
            foreach ($this->data as $v) {
                if (!isset($v->cl_status) || $v->cl_status === 'visible') {
                    ...
                    if ($v->type == 'file') {
                    ...
                    } else if ($v->type == 'number' && isset($_REQUEST[$v->name]) && !empty($_REQUEST[$v->name]) ) {
                        $val = $_REQUEST[$v->name];
                        ...
                        if (isset($_REQUEST[$v->name])) {
                            ...
                            if (!empty($new_product_array)) {
                                $_REQUEST[$v->name.'_products'] = $new_product_array;
...
//            $field = 'attribute';
        ...
        } else if ($rules->cl_field === 'custom_attribute') {
            $field = (object) ['type' => 'custom_attribute'];
            $product = wc_get_product($product_id);
        ...
        if ($field) {
            ...
            if ($is_visible) {
                ...
                } else if ($field->type === 'custom_attribute') {
                    $name = 'attribute_' . $rules->cl_relation;
                    if (isset($_REQUEST[$name])) {
                        $val[] = strtolower($_REQUEST[$name]);
                    }
            ...
            if (in_array($field->type, ['attribute', 'custom_attribute', 'custom_field'])) {
    ...
    public function get_pro_attr_list($pro = false)
    {
        ...
        foreach ($attributes as $attribute) {
            ...
            if ($attribute->is_taxonomy()) {
                 ...
                 */
                if ( $attribute->get_variation() && $pro->is_type('variable') ) {
                        ...
                        if (is_array($_v)) {
                            foreach ($v->values as $newkey => $_v2) {
                                if ((isset($_v2->value) && ($_v2->value === $_v['value']
                                    ...
                                    || (isset($_v2->image) && isset($_v['image']) && $_v2->image === $_v['image'])) {
                                    $flag = true;
                                    break;
                                }
                            }
...
//        $unixTimestamp = strtotime($value);
        ...
        if(wc_get_price_decimal_separator() ==',' ){
            preg_match('/\d*\,?\d*$/m', $value, $_val);
            if(isset($_val) && isset($_val[0])){
                $value= str_replace($_val[0], str_replace(',', '.', $_val[0]), $value);
            }
        }
...
//                                    $unixTimestamp = strtotime($sub_data['value']);
    ...
    public function submited_data($product_id, $variation_id = 0)
    {
        ...
        if(!($is_variation || $is_grouped)){
            if (isset($_REQUEST['add-to-cart'])) {
                if (intval($_REQUEST['add-to-cart']) != $product_id) {
                    return $this->submited_data;
                }
            } else if(isset($_REQUEST['product_id'])) {
                if (intval($_REQUEST['product_id']) != $product_id) {
                    return $this->submited_data;
                }
            }
        }
            ...
            if (!in_array($v->type, array('separator')) && (!isset($v->cl_status) || $v->cl_status == 'visible')) {
                ...
                if ($v->type == 'file') {
                    ...
                    if (in_array($v->type, ['select', 'radio-group'])) {
                        ...
                        $products = $_REQUEST[$v->name.'_products'];
                        $value_objects = [];
                        
                        $field_unique_key = ''; // Fix the cart key generation issue (issue:cartkey generating same for different product group options for same product)
                        ...
                        if (!empty($quantities)) {
                            if (count($quantities) > 0) {
                                foreach ($quantities as $k => $quantity) {
                                    if (!is_object($products[$k])) {
                                        $value_objects[$k] = wc_get_product($values[$k]);
                                        $field_unique_key .= $values[$k].'_';
                                    } {
                                        $value_objects[$k] = $products[$k];
                                        $field_unique_key .= $products[$k]->get_id().'_';
                                    }
                                }
                                $this->submited_data[] = array(
                                    'type' => $v->type,
                                    ...
                                    'field_unique_key' => rtrim($field_unique_key , '_'),
                                    ...
                                    'is_fee' => (isset($v->use_as_fee) && $v->use_as_fee) ? true : false,
                                    'is_show_price' => (isset($v->is_show_price) && $v->is_show_price) ? true : false,
                                    'price' => $this->element_price($v, $product_id, array('values' => $value_objects, 'quantities' => $quantities)),
                                    'quantity_depend' => $this->is_quantity_dependent($v, $product_id, $value_objects),
                                    'cur_swit' => $this->getCurrSwitch(),
                                    'form_data' => $form_data,
                                );
                            }
    ...
    public function render($after_acb = false, $is_rest = false)
    {
        ...
        if (isset($this->relations['attribute'])
            || isset($this->relations['custom_attribute'])
            ...
            $attr_related = 'data-attrrelated=\'' . json_encode(array_merge(
                isset($this->relations['attribute']) ? $this->relations['attribute'] : [],
                isset($this->relations['custom_attribute']) ? $this->relations['custom_attribute'] : [],
        ...
        $productPrice = apply_filters('raw_woocommerce_price', wcpa_get_price_shop($this->product));
        $filteredProductPrice = apply_filters('wcpa_product_price', $productPrice, $this->product);
        $productPrice = floatval(is_numeric($filteredProductPrice) ? $filteredProductPrice :  $productPrice);

        //Compatibility Discount Rules By Flycart
        $dicounted_price = false;
        if (class_exists('\Wdr\App\Router')) {
            $dicounted_price = apply_filters('advanced_woo_discount_rules_get_product_discount_price_from_custom_price', $productPrice, $this->product, 1, 0, 'discounted_price', true, false);
        }
        
        $product_data['wc_product_price'] = $dicounted_price != false ? $productPrice : $productPrice / $this->get_con_unit(true);
        ...
        $product_data['is_grouped'] = $this->product->is_type('grouped') ? true : false;
        $product_data['is_external'] = $this->product->is_type('external') ? true : false;
        ...
        //Get Value From Cart While Editing the product 
        $default_values = [];
        $edit_cart_item = false;
        if(isset($_GET['cart_key']) && !empty($_GET['cart_key'])) {
            if(wcpa_check_woocommerce_active()) {
                $cart_key = sanitize_text_field($_GET['cart_key']);
                $cartItem = WC()->cart->get_cart_item( $cart_key );
                if(
                    isset($cartItem) && !empty($cartItem) &&
                    isset($cartItem[WCPA_CART_ITEM_KEY]) && !empty($cartItem[WCPA_CART_ITEM_KEY]) 
                ) {
                    $edit_cart_item = true;
                    foreach($cartItem[WCPA_CART_ITEM_KEY] as $fieldData) {
                        $default_values[$fieldData['name']] = $fieldData['value'];
                    }

                    if(isset($cartItem['quantity'])) {
                        add_filter( 'woocommerce_quantity_input_args', function ($args, $product) use ($cartItem) {
        ...
        if($edit_cart_item) {
            $cart_key = sanitize_text_field($_GET['cart_key']);
            echo '<input type="hidden" name="wcpa_current_cart_key" value="'.$cart_key.'" />';
        }
        ...
        foreach ($this->data as $v) {
            ...
            if(
                ...
                $default_value = isset($default_values[$v->name])? $default_values[$v->name] : '';
            } else {
                $default_value = false;
            }
            ...
            switch ($v->type) {
                case 'text':
                    $this->render_text($v, $default_value);
                    break;
                case 'hidden':
                    $this->render_hidden($v, $default_value);
                    break;
                case 'color':
                    $this->render_color($v, $default_value);
                    $this->enqueue_colorpicker();
                    break;
                case 'number':
                    $this->render_number($v, $default_value);
                    break;
                case 'date':
                    $this->render_date($v, $default_value);

                    $this->enqueue_detetimepicker();
                    break;
                case 'time':
                    $this->render_time($v, $default_value);
                    $this->enqueue_detetimepicker();
                    break;
                case 'datetime-local':
                    $this->render_datetime($v, $default_value);
                    $this->enqueue_detetimepicker();
                    break;
                case 'checkbox-group':
                    $this->render_checkbox($v, $default_value);

                    break;
                case 'radio-group':
                    $this->render_radio($v, $default_value);
                    break;
                case 'header':
                    $this->render_header($v);
                    break;
                case 'file':
                    $this->render_file($v, $default_value);
                    break;
                case 'paragraph':
                ...
                case 'statictext':
                    ...
                    break;
                case 'productGroup':
                    $this->render_productGroup($v, $default_value);
                    break;
                case 'separator':
                    $this->render_separator($v);
                    break;
                case 'select':
                    $this->render_select($v, $default_value);
                    break;
                case 'textarea':
                    $this->render_textarea($v, $default_value);
                    break;
                case 'image-group':
                    $this->render_image($v, $default_value);
                    break;
                case 'color-group':
                    $this->render_colorgroup($v, $default_value);
                    break;
                case 'placeselector':
                    $this->render_place($v, $default_value);
    ...
    public function get_con_unit($toMultiplyWithShopPrice = false, $v = false)
    {
        ...
        //Compatibility for Discount Rules by Flycart
        if (class_exists('\Wdr\App\Router')) {
            $productPrice = wcpa_get_price_shop($this->product);
            $dicounted_price = apply_filters('advanced_woo_discount_rules_get_product_discount_price_from_custom_price', $productPrice, $this->product, 1, 0, 'discounted_price', true, false);
            if ($this->conversion_unit[$index] === false && $dicounted_price != false) {
                if($productPrice) {
                    $this->conversion_unit[$index] = $dicounted_price/$productPrice;
    ...
    public function render_text($v, $default)
    {
        $data = '';
        $maxlength = '';
        $minlength = '';
        ...
        $validation .= htmlspecialchars(json_encode($validation_array), ENT_QUOTES) . '\'';
        ...
        . ' ' . $className . ''
        . ' name="' . $name . '"'
        . ' value="' . $this->default_value($v, $default) . '" '
            . '' . $maxlength . ' '
            . '' . $minlength . ' '
            . '' . $txt_required . ''
        ...
        if (isset($v->charleft) && $v->charleft && isset($v->maxlength) && !empty($v->maxlength)) {
            $char_count = (int) $v->maxlength - strlen($this->default_value($v, $default));
            if ($char_count >= 0) {
    ...
    private function default_value($v, $default=false)
    {
        ...
        switch ($v->type) {
            ...
            case 'datetime-local':
                if ($this->cart_error && (isset($v->name)) && isset($_POST[$v->name])) { // if there is a validation error, it has persist the user entered values,
                    ...
                    $default_value = $this->sanitize_values($v);
                } elseif($default == false && isset($v->value) ) { //if no default value passed
            ...
            case 'productGroup':
                ...
                if ($this->cart_error && (isset($v->name)) && isset($_POST[$v->name])) { // if there is a validation error, it has persist the user entered values,
                    ...
                    $default_value = $this->sanitize_values($v);
                } else if ($default == false && $v->values && !(isset($_REQUEST['add-to-cart']) && $this->cart_error)) { // if it is direct product page load, not add-to-cart has set & if no default value passed
                    foreach ($v->values as $k => $val) {
                        if (isset($val->selected)) {
                            $default_value[$k] = ($v->type=='select') ? $val->value : $val->selected;
                        }
                    }
                } else if (isset($default) && !empty($default) && $v->values && !(isset($_REQUEST['add-to-cart']) && $this->cart_error)) { // if it is direct product page load, not add-to-cart has set &  if default value passed
                    foreach ($v->values as $k => $val) {
                        foreach($default as $dval) {
                            if($v->type=='productGroup') {
                                if($dval->get_id()==$val->value) {
                            ...
                            } else if($val->value == $dval['value']) {
                                $default_value[$k] = $dval['value'];
                            }
                        }
                    }
                }
    ...
    public function render_hidden($v, $default)
    {
        $name = (isset($v->name)) ? $v->name : '';
        $value = (isset($v->value)) ? $v->value : '';

        echo '<input type="hidden"  id="' . $name . '"  name="' . $name . '" value="' . $this->default_value($v, $default) . '" />';
    }

    public function render_color($v, $default)
    {
        $data = '';
        $maxlength = '';
        $txt_required = (isset($v->required)) ? 'required="required"' : '';
        ...
        echo '<input type="color"  id="' . $name . '" ' . $placeholder . ' ' . $className . ' name="' . $name . '" value="' . $this->default_value($v, $default) . '" ' . $txt_required . ' ' . $price_data . '  ' . $validation . '  />';
    }
    ...
    public function render_number($v, $default)
    {
        ...
        echo '<input ' . ((isset($v->disable_manual_input) && $v->disable_manual_input) ? 'readonly="readonly"' : '') . '   type="' . $v->type . '"  id="' . $name . '" ' . $placeholder . ' ' . $className . ' name="' . $name . '" value="' . $this->default_value($v, $default) . '" ' . $max . ' ' . $min . ' ' . $step . ' ' . $num_required . ' ' . $price_data . ' ' . $pattern . ' ' . $validation . ' />';
    ...
    public function render_date($v, $default)
    {
        ...
        echo '<input autocomplete="off" ' . ((isset($v->disable_manual_input) && $v->disable_manual_input) ? 'readonly="readonly"' : '') . ' type="text" ' . $price_data . ' id="' . $name . '" ' . $placeholder . ' ' . $className . ' name="' . $name . '" value="' . $this->default_value($v, $default) . '" ' . $required . ' ' . $config_data . ' ' . $validation . ' />';
    }
    ...
    public function render_time($v, $default)
    {
        $required = (isset($v->required)) ? 'required="required"' : '';
        $placeholder = (isset($v->placeholder)) ? 'placeholder="' . $v->placeholder . '"' : '';
    ...
    public function render_datetime($v, $default)
    {
        $required = (isset($v->required)) ? 'required="required"' : '';
        $placeholder = (isset($v->placeholder)) ? 'placeholder="' . $v->placeholder . '"' : '';
    ...
    public function render_checkbox($v, $default)
    {
        ...
        $default_value = $this->default_value($v, $default);
    ...
    {
        $replace_sign = (substr($price, 0, 1) == '-') ? '' : '+';
        return str_replace('{sign}',$replace_sign,str_replace('price', $price, wcpa_get_option('field_option_price_format', '(price)')));
    }
    ...
    public function get_pimage_data($v, $val)
    {
        ...
        if (isset($v->enable_product_image) && $v->enable_product_image) {
            if (isset($val->pimage_id) && $val->pimage_id > 0) {
                ...
                $image_data = 'data-image=\'' . json_encode($attachProps + ['image_id' => $val->pimage_id]) . '\'';
            } else if ($val->pimage) {
                $props = [
    ...
    public function render_radio($v, $default)
    {
        global $wcpa_field_counter;
        $name = (isset($v->name)) ? $v->name : '';
        $this->label_section($v);
    ...
    public function render_file($v, $default)
    {
        $data = '';
        $maxlength = '';
        $txt_required = (isset($v->required)) ? 'required="required"' : '';
    ...
    public function render_productGroup($v, $default)
    {
        global $wcpa_field_counter;
        $name = (isset($v->name)) ? $v->name : '';
        $this->label_section($v);
            ...
            foreach ($v->values as $k => $val) {
                ...
                if (
                    ...
                    if (!$val_error) {
                        ...
                        $label = '<span class="wcpa_option_label">';

                        $productGroup_option_label= (
                                isset($v->custom_label) && $v->custom_label && 
                                isset($val->custom_label) && !empty($val->custom_label)
                            ) 
                                ? $val->custom_label 
                                : $addon_product->get_title();
                        $label .= $productGroup_option_label;
                        $imgMagLabel = $productGroup_option_label;
                        ...
                        if (isset($v->enablePrice) && $v->enablePrice) {
                            if ((isset($v->form_rules['disp_hide_options_price']) && $v->form_rules['disp_hide_options_price'] === false)) {
                                ...
                                if ($price !== false) {
                                    ...
                                    if (!(($price - 0) < 0.0000001 && wcpa_get_option('wcpa_hide_option_price_zero', false))) {
                                        $wcpa_opt_price = $this->field_option_price(wcpa_price($price * $this->get_con_unit(true, $v), 1));
                                        $label .= ' <span class="wcpa_opt_price">' . $wcpa_opt_price.'</span>';
                                        $imgMagLabel .= ' '.$wcpa_opt_price;
                                    }
                                }
                        ...
                        if (isset($v->show_image) && $v->show_image) {
                            ...
                            if ($pro_image !== '') {
                                ...
                                <span class="wcpa_img_mag" data-src="' . $pro_image . '" data-title="' . $imgMagLabel . '">+</span>';
    ...
    public function render_select($v, $default)
    {
    ...
    public function render_textarea($v, $default)
    {
        $maxlength = '';
        ...
        echo '<textarea ' . ((isset($v->disable_manual_input) && $v->disable_manual_input) ? 'readonly="readonly"' : '') . ' ' . $validation . ' id="' . $name . '"  ' . $rows . ' ' . $placeholder . ' ' . $className . ' name="' . $name . '" ' . $maxlength . ' ' . $minlength . ' ' . $title . ' ' . $txtarea_required . ' ' . $price_data . ' >' . $this->default_value($v, $default) . '</textarea>';
    ...
    public function render_image($v, $default)
    {
        global $wcpa_field_counter;
        $name = (isset($v->name)) ? $v->name : '';
        $this->label_section($v);
            ...
            foreach ($v->values as $k => $val) {
                ...
                if (isset($v->show_as_product_image) && $v->show_as_product_image) {
                    ...
                    if (isset($val->image_id) && $val->image_id > 0) {
                        ...
                        $image_data = 'data-image=\'' . json_encode($attachProps + ['image_id' => $val->image_id]) . '\'';
                    } else {
                        $props = [
                            'title' => htmlspecialchars($val->label, ENT_QUOTES),
                            'caption' => '',
    ...
    public function render_colorgroup($v, $default)
    {
        global $wcpa_field_counter;
        $name = (isset($v->name)) ? $v->name : '';
        $this->label_section($v);
...
//                    $is_selected = ($default_value === (string) $k) ? true : false;
    ...
    public function render_place($v, $default)
    {
