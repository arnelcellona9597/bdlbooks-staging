 ...
 */
	...
	*/
	...
	define('WPO_WCLABELS_QR_ECLEVEL_L', 0); 
	/** ~15% of codewords can be restored */
	define('WPO_WCLABELS_QR_ECLEVEL_M', 1); 
	/** ~25% of codewords can be restored */
	define('WPO_WCLABELS_QR_ECLEVEL_Q', 2);
	/** ~30% of codewords can be restored */
	define('WPO_WCLABELS_QR_ECLEVEL_H', 3);
	/** @}*/
   
	/** @name QR-Code Supported output formats */
	/** @{*/
	define('WPO_WCLABELS_QR_FORMAT_TEXT', 0);
	define('WPO_WCLABELS_QR_FORMAT_PNG',  1);
	/** @}*/
 ...
 */
	...
	define('WPO_WCLABELS_QR_CACHEABLE', false);       // use cache - more disk reads but less CPU power, masks and format templates are stored there
	define('WPO_WCLABELS_QR_CACHE_DIR', false);       // used when WPO_WCLABELS_QR_CACHEABLE === true
	define('WPO_WCLABELS_QR_LOG_DIR', false);         // default error logs dir   
	
	define('WPO_WCLABELS_QR_FIND_BEST_MASK', true);                                                          // if true, estimates best mask (spec. default, but extremally slow; set to false to significant performance boost but (propably) worst quality code
	define('WPO_WCLABELS_QR_FIND_FROM_RANDOM', 2);                                                       // if false, checks all masks available, otherwise value tells count of masks need to be checked, mask id are got randomly
	define('WPO_WCLABELS_QR_DEFAULT_MASK', 2);                                                               // when WPO_WCLABELS_QR_FIND_BEST_MASK === false
												  
	define('WPO_WCLABELS_QR_PNG_MAXIMUM_SIZE',  1024);                                                       // maximum allowed png image width (in pixels), tune to make sure GD and PHP can handle such big images
 ...
 */
	...
	class WPO_WCLABELS_QRtools {
		...
		public static function tcpdfBarcodeArray($code, $mode = 'QR,L', $tcPdfVersion = '4.5.037')
		{
			...
			$qrTab = WPO_WCLABELS_QRcode::text($code, false, $eccLevel);
			$size = count($qrTab);
		...
		{
			WPO_WCLABELS_QRtools::markTime('before_build_cache');
			
			$mask = new WPO_WCLABELS_QRmask();
			for ($a=1; $a <= WPO_WCLABELS_QRSPEC_VERSION_MAX; $a++) {
				$frame = WPO_WCLABELS_QRspec::newFrame($a);
				if (WPO_WCLABELS_QR_IMAGE) {
					$fileName = WPO_WCLABELS_QR_CACHE_DIR.'frame_'.$a.'.png';
					WPO_WCLABELS_QRimage::png(self::binarize($frame), $fileName, 1, 0);
				}
			...
			WPO_WCLABELS_QRtools::markTime('after_build_cache');
		}

		//----------------------------------------------------------------------
		public static function log($outfile, $err)
		{
			if (WPO_WCLABELS_QR_LOG_DIR !== false) {
				if ($err != '') {
					if ($outfile !== false) {
						file_put_contents(WPO_WCLABELS_QR_LOG_DIR.basename($outfile).'-errors.txt', date('Y-m-d H:i:s').': '.$err, FILE_APPEND);
					} else {
						file_put_contents(WPO_WCLABELS_QR_LOG_DIR.'errors.txt', date('Y-m-d H:i:s').': '.$err, FILE_APPEND);
					}
				}    
			}
		}
 ...
 */
	...
	define('WPO_WCLABELS_QRSPEC_VERSION_MAX', 40);
	/** Maximal Code size in pixels allowed by QR-Code spec */
	define('WPO_WCLABELS_QRSPEC_WIDTH_MAX',   177);

	define('WPO_WCLABELS_QRCAP_WIDTH',        0);
	define('WPO_WCLABELS_QRCAP_WORDS',        1);
	define('WPO_WCLABELS_QRCAP_REMINDER',     2);
	define('WPO_WCLABELS_QRCAP_EC',           3);
	...
	*/
	class WPO_WCLABELS_QRspec {
		...
		*/
		...
		{
			return self::$capacity[$version][WPO_WCLABELS_QRCAP_WORDS] - self::$capacity[$version][WPO_WCLABELS_QRCAP_EC][$level];
		}
		...
		*/
		...
		{
			return self::$capacity[$version][WPO_WCLABELS_QRCAP_EC][$level];
		}
		...
		*/
		...
		{
			return self::$capacity[$version][WPO_WCLABELS_QRCAP_WIDTH];
		}
		...
		*/
		...
		{
			return self::$capacity[$version][WPO_WCLABELS_QRCAP_REMINDER];
		}
		...
		*/
		public static function getMinimumVersion($size, $level)
		{
			...
			for($i=1; $i<= WPO_WCLABELS_QRSPEC_VERSION_MAX; $i++) {
				$words  = self::$capacity[$i][WPO_WCLABELS_QRCAP_WORDS] - self::$capacity[$i][WPO_WCLABELS_QRCAP_EC][$level];
				if($words >= $size) 
					return $i;
			}
		...
		*/
		...
		{
			if ($mode == WPO_WCLABELS_QR_MODE_STRUCTURE)
				return 0;
		...
		public static function maximumWords($mode, $version)
		{
			...
			if($mode == WPO_WCLABELS_QR_MODE_KANJI) {
		...
		*/
		...
		size: [WPO_WCLABELS_QRSPEC_VERSION_MAX - 6]
		...
		*/
		...
		{
			if($version < 7 || $version > WPO_WCLABELS_QRSPEC_VERSION_MAX)
				return 0;
		...
		*/
		...
		{
			$width = self::$capacity[$version][WPO_WCLABELS_QRCAP_WIDTH];
		...
		*/
		...
		{
			if($version < 1 || $version > WPO_WCLABELS_QRSPEC_VERSION_MAX) 
				return null;

			if(!isset(self::$frames[$version])) {
				
				$fileName = WPO_WCLABELS_QR_CACHE_DIR.'frame_'.$version.'.dat';
				
				if (WPO_WCLABELS_QR_CACHEABLE) {
 ...
 */
	...
	define('WPO_WCLABELS_QR_IMAGE', true);
 ...
 */
	...
	define('WPO_WCLABELS_STRUCTURE_HEADER_BITS',  20);
	define('WPO_WCLABELS_MAX_STRUCTURED_SYMBOLS', 16);
	
	/** @addtogroup CoreGroup */
	/** @{ */

	class WPO_WCLABELS_QRinputItem {
		...
		public function __construct($mode, $size, $data, $bstream = null) 
		{
			...
			if(!WPO_WCLABELS_QRinput::check($mode, $size, $setData)) {
		...
		public function encodeModeNum($version)
		{
			try {
				...
				$bs = new WPO_WCLABELS_QRbitstream();
				
				$val = 0x1;
				$bs->appendNum(4, $val);
				$bs->appendNum(WPO_WCLABELS_QRspec::lengthIndicator(WPO_WCLABELS_QR_MODE_NUM, $version), $this->size);
		...
		public function encodeModeAn($version)
		{
			try {
				...
				$bs->appendNum(4, 0x02);
				$bs->appendNum(WPO_WCLABELS_QRspec::lengthIndicator(WPO_WCLABELS_QR_MODE_AN, $version), $this->size);

				for($i=0; $i<$words; $i++) {
					$val  = (int)WPO_WCLABELS_QRinput::lookAnTable(ord($this->data[$i*2  ])) * 45;
					$val += (int)WPO_WCLABELS_QRinput::lookAnTable(ord($this->data[$i*2+1]));

					$bs->appendNum(11, $val);
				}

				if($this->size & 1) {
					$val = WPO_WCLABELS_QRinput::lookAnTable(ord($this->data[$words * 2]));
					$bs->appendNum(6, $val);
				}
		...
		public function encodeMode8($version)
		{
			try {
				...
				$bs->appendNum(4, 0x4);
				$bs->appendNum(WPO_WCLABELS_QRspec::lengthIndicator(WPO_WCLABELS_QR_MODE_8, $version), $this->size);
		...
		public function encodeModeKanji($version)
		{
			try {
				...
				$bs->appendNum(4, 0x8);
				$bs->appendNum(WPO_WCLABELS_QRspec::lengthIndicator(WPO_WCLABELS_QR_MODE_KANJI, $version), (int)($this->size / 2));
		...
		public function estimateBitStreamSizeOfEntry($version)
		{
			...
			switch($this->mode) {
				case WPO_WCLABELS_QR_MODE_NUM:        $bits = WPO_WCLABELS_QRinput::estimateBitsModeNum($this->size);    break;
				case WPO_WCLABELS_QR_MODE_AN:        $bits = WPO_WCLABELS_QRinput::estimateBitsModeAn($this->size);    break;
				case WPO_WCLABELS_QR_MODE_8:            $bits = WPO_WCLABELS_QRinput::estimateBitsMode8($this->size);    break;
				case WPO_WCLABELS_QR_MODE_KANJI:        $bits = WPO_WCLABELS_QRinput::estimateBitsModeKanji($this->size);break;
				case WPO_WCLABELS_QR_MODE_STRUCTURE:    return WPO_WCLABELS_STRUCTURE_HEADER_BITS;            
				default:
					return 0;
			}

			$l = WPO_WCLABELS_QRspec::lengthIndicator($this->mode, $version);
			$m = 1 << $l;
		...
		public function encodeBitStream($version)
		{
			try {
				...
				unset($this->bstream);
				$words = WPO_WCLABELS_QRspec::maximumWords($this->mode, $version);
				
				if($this->size > $words) {
				
					$st1 = new WPO_WCLABELS_QRinputItem($this->mode, $words, $this->data);
					$st2 = new WPO_WCLABELS_QRinputItem($this->mode, $this->size - $words, array_slice($this->data, $words));

					$st1->encodeBitStream($version);
					$st2->encodeBitStream($version);
					
					$this->bstream = new WPO_WCLABELS_QRbitstream();
					...
					switch($this->mode) {
						case WPO_WCLABELS_QR_MODE_NUM:        $ret = $this->encodeModeNum($version);    break;
						case WPO_WCLABELS_QR_MODE_AN:        $ret = $this->encodeModeAn($version);    break;
						case WPO_WCLABELS_QR_MODE_8:            $ret = $this->encodeMode8($version);    break;
						case WPO_WCLABELS_QR_MODE_KANJI:        $ret = $this->encodeModeKanji($version);break;
						case WPO_WCLABELS_QR_MODE_STRUCTURE:    $ret = $this->encodeModeStructure();    break;
	...
	class WPO_WCLABELS_QRinput {
		...
		//----------------------------------------------------------------------
		public function __construct($version = 0, $level = WPO_WCLABELS_QR_ECLEVEL_L)
		{
			if ($version < 0 || $version > WPO_WCLABELS_QRSPEC_VERSION_MAX || $level > WPO_WCLABELS_QR_ECLEVEL_H) {
		...
		{
			if($version < 0 || $version > WPO_WCLABELS_QRSPEC_VERSION_MAX) {
				throw new Exception('Invalid version no');
				return -1;
			}
		...
		{
			if($level > WPO_WCLABELS_QR_ECLEVEL_H) {
		...
		//----------------------------------------------------------------------
		public function appendEntry(WPO_WCLABELS_QRinputItem $entry)
		{
			$this->items[] = $entry;
		}
		...
		{
			try {
				$entry = new WPO_WCLABELS_QRinputItem($mode, $size, $data);
				$this->items[] = $entry;
				return 0;
			} catch (Exception $e) {
				return -1;
		...
		{
			if( $size > WPO_WCLABELS_MAX_STRUCTURED_SYMBOLS ) {
				throw new Exception('insertStructuredAppendHeader wrong size');
			}
			
			if( $index <= 0 || $index > WPO_WCLABELS_MAX_STRUCTURED_SYMBOLS ) {
			...
			try {
				$entry = new WPO_WCLABELS_QRinputItem(WPO_WCLABELS_QR_MODE_STRUCTURE, 3, $buf);
		...
		public function calcParity()
		{
			...
			foreach($this->items as $item) {
				if($item->mode != WPO_WCLABELS_QR_MODE_STRUCTURE) {
		...
		public static function check($mode, $size, $data)
		{
			...
			switch($mode) {
				case WPO_WCLABELS_QR_MODE_NUM:       return self::checkModeNum($size, $data);   break;
				case WPO_WCLABELS_QR_MODE_AN:        return self::checkModeAn($size, $data);    break;
				case WPO_WCLABELS_QR_MODE_KANJI:     return self::checkModeKanji($size, $data); break;
				case WPO_WCLABELS_QR_MODE_8:         return true; break;
				case WPO_WCLABELS_QR_MODE_STRUCTURE: return true; break;
		...
		public function estimateVersion()
		{
			...
			do {
				...
				$version = WPO_WCLABELS_QRspec::getMinimumVersion((int)(($bits + 7) / 8), $this->level);
				if ($version < 0) {
					return -1;
				}
			} while ($version > $prev);
		...
		{
			$payload = $bits - 4 - WPO_WCLABELS_QRspec::lengthIndicator($mode, $version);
			switch($mode) {
				case WPO_WCLABELS_QR_MODE_NUM:
				...
				case WPO_WCLABELS_QR_MODE_AN:
					...
					if($remain >= 6) 
						$size++;
					break;
				case WPO_WCLABELS_QR_MODE_8:
					$size = (int)($payload / 8);
					break;
				case WPO_WCLABELS_QR_MODE_KANJI:
					$size = (int)(($payload / 13) * 2);
					break;
				case WPO_WCLABELS_QR_MODE_STRUCTURE:
					$size = (int)($payload / 8);
					break;
				default:
					$size = 0;
			...
			$maxsize = WPO_WCLABELS_QRspec::maximumWords($mode, $version);
			if($size < 0) $size = 0;
		...
		public function convertData()
		{
			...
			for(;;) {
				...
				$ver = WPO_WCLABELS_QRspec::getMinimumVersion((int)(($bits + 7) / 8), $this->level);
				if($ver < 0) {
		...
		{
			$bits = $bstream->size();
			$maxwords = WPO_WCLABELS_QRspec::getDataLength($this->version, $this->level);
			$maxbits = $maxwords * 8;
			...
			$padding = new WPO_WCLABELS_QRbitstream();
		...
		public function mergeBitStream()
		{
			...
			$bstream = new WPO_WCLABELS_QRbitstream();
 ...
 */
	...
	*/
	class WPO_WCLABELS_QRbitstream {
		...
		*/
		...
		@return WPO_WCLABELS_QRbitstream bit stream object containing first $bits bits from $num in order from LSB to MSB
		*/
		...
		@return WPO_WCLABELS_QRbitstream bit stream object containing bit contents of given bytes array
		*/
		...
		@param WPO_WCLABELS_QRbitstream $arg bit stream to be appended
		@return Integer status of append operation, 0 when success, -1 when $arg is null
		*/
		public function append(WPO_WCLABELS_QRbitstream $arg)
		{
			if (is_null($arg)) {
				return -1;
			}
		...
		*/
		public function appendNum($bits, $num)
		{
			...
			$b = WPO_WCLABELS_QRbitstream::newFromNum($bits, $num);
		...
		*/
		public function appendBytes($size, $data)
		{
			...
			$b = WPO_WCLABELS_QRbitstream::newFromBytes($size, $data);
 ...
 */
	...
	class WPO_WCLABELS_QRsplit {
		...
		public static function isalnumat($str, $pos)
		{
			...
			return (WPO_WCLABELS_QRinput::lookAnTable(ord($str[$pos])) >= 0);
		}
		...
		public function identifyMode($pos)
		{
			if ($pos >= strlen($this->dataStr)) 
				return WPO_WCLABELS_QR_MODE_NUL;
				
			$c = $this->dataStr[$pos];
			
			if(self::isdigitat($this->dataStr, $pos)) {
				return WPO_WCLABELS_QR_MODE_NUM;
			} else if(self::isalnumat($this->dataStr, $pos)) {
				return WPO_WCLABELS_QR_MODE_AN;
			} else if($this->modeHint == WPO_WCLABELS_QR_MODE_KANJI) {
				...
				if ($pos+1 < strlen($this->dataStr)) 
				{
					...
					if(($word >= 0x8140 && $word <= 0x9ffc) || ($word >= 0xe040 && $word <= 0xebbf)) {
						return WPO_WCLABELS_QR_MODE_KANJI;
					}
				}
			}

			return WPO_WCLABELS_QR_MODE_8;
		} 
		
		//----------------------------------------------------------------------
		public function eatNum()
		{
			$ln = WPO_WCLABELS_QRspec::lengthIndicator(WPO_WCLABELS_QR_MODE_NUM, $this->input->getVersion());
			...
			if($mode == WPO_WCLABELS_QR_MODE_8) {
				$dif = WPO_WCLABELS_QRinput::estimateBitsModeNum($run) + 4 + $ln
					 + WPO_WCLABELS_QRinput::estimateBitsMode8(1)         // + 4 + l8
					 - WPO_WCLABELS_QRinput::estimateBitsMode8($run + 1); // - 4 - l8
				if($dif > 0) {
					return $this->eat8();
				}
			}
			if($mode == WPO_WCLABELS_QR_MODE_AN) {
				$dif = WPO_WCLABELS_QRinput::estimateBitsModeNum($run) + 4 + $ln
					 + WPO_WCLABELS_QRinput::estimateBitsModeAn(1)        // + 4 + la
					 - WPO_WCLABELS_QRinput::estimateBitsModeAn($run + 1);// - 4 - la
				if($dif > 0) {
					return $this->eatAn();
				}
			}
			
			$ret = $this->input->append(WPO_WCLABELS_QR_MODE_NUM, $run, str_split($this->dataStr));
			if($ret < 0)
				return -1;
		...
		{
			$la = WPO_WCLABELS_QRspec::lengthIndicator(WPO_WCLABELS_QR_MODE_AN,  $this->input->getVersion());
			$ln = WPO_WCLABELS_QRspec::lengthIndicator(WPO_WCLABELS_QR_MODE_NUM, $this->input->getVersion());
			...
			while(self::isalnumat($this->dataStr, $p)) {
				if(self::isdigitat($this->dataStr, $p)) {
					...
					$dif = WPO_WCLABELS_QRinput::estimateBitsModeAn($p) // + 4 + la
						 + WPO_WCLABELS_QRinput::estimateBitsModeNum($q - $p) + 4 + $ln
						 - WPO_WCLABELS_QRinput::estimateBitsModeAn($q); // - 4 - la
			...
			if(!self::isalnumat($this->dataStr, $p)) {
				$dif = WPO_WCLABELS_QRinput::estimateBitsModeAn($run) + 4 + $la
					 + WPO_WCLABELS_QRinput::estimateBitsMode8(1) // + 4 + l8
					  - WPO_WCLABELS_QRinput::estimateBitsMode8($run + 1); // - 4 - l8
				if($dif > 0) {
					return $this->eat8();
			...
			$ret = $this->input->append(WPO_WCLABELS_QR_MODE_AN, $run, str_split($this->dataStr));
			if($ret < 0)
				return -1;
		...
		public function eatKanji()
		{
			...
			while($this->identifyMode($p) == WPO_WCLABELS_QR_MODE_KANJI) {
				$p += 2;
			}
			
			$ret = $this->input->append(WPO_WCLABELS_QR_MODE_KANJI, $p, str_split($this->dataStr));
			if($ret < 0)
				return -1;
		...
		public function eat8()
		{
			...
			while($p < $dataStrLen) {
				...
				if($mode == WPO_WCLABELS_QR_MODE_NUM) {
					$q = $p;
					while(self::isdigitat($this->dataStr, $q)) {
						$q++;
					}
					$dif = WPO_WCLABELS_QRinput::estimateBitsMode8($p) // + 4 + l8
						 + WPO_WCLABELS_QRinput::estimateBitsModeNum($q - $p) + 4 + $ln
						 - WPO_WCLABELS_QRinput::estimateBitsMode8($q); // - 4 - l8
					if($dif < 0) {
						break;
					} else {
						$p = $q;
					}
				} else if($mode == WPO_WCLABELS_QR_MODE_AN) {
					$q = $p;
					while(self::isalnumat($this->dataStr, $q)) {
						$q++;
					}
					$dif = WPO_WCLABELS_QRinput::estimateBitsMode8($p)  // + 4 + l8
						 + WPO_WCLABELS_QRinput::estimateBitsModeAn($q - $p) + 4 + $la
						 - WPO_WCLABELS_QRinput::estimateBitsMode8($q); // - 4 - l8
					if($dif < 0) {
						break;
					} else {
			...
			$run = $p;
			$ret = $this->input->append(WPO_WCLABELS_QR_MODE_8, $run, str_split($this->dataStr));
		...
		public function splitString()
		{
			while (strlen($this->dataStr) > 0)
			{
				...
				switch ($mode) {
					case WPO_WCLABELS_QR_MODE_NUM: $length = $this->eatNum(); break;
					case WPO_WCLABELS_QR_MODE_AN:  $length = $this->eatAn(); break;
					case WPO_WCLABELS_QR_MODE_KANJI:
						if ($hint == WPO_WCLABELS_QR_MODE_KANJI)
		...
		//----------------------------------------------------------------------
		public static function splitStringToQRinput($string, WPO_WCLABELS_QRinput $input, $modeHint, $casesensitive = true)
		{
			if(is_null($string) || $string == '\0' || $string == '') {
				throw new Exception('empty string!!!');
			}

			$split = new WPO_WCLABELS_QRsplit($string, $input, $modeHint);
	...
	class WPO_WCLABELS_QRrs {
		...
		*/
		public static function init_rs($symsize, $gfpoly, $fcr, $prim, $nroots, $pad)
		{
			...
			$rs = WPO_WCLABELS_QRrsItem::init_rs_char($symsize, $gfpoly, $fcr, $prim, $nroots, $pad);
 ...
 */
	...
	define('WPO_WCLABELS_N1', 3);
	define('WPO_WCLABELS_N2', 3);
	define('WPO_WCLABELS_N3', 40);
	define('WPO_WCLABELS_N4', 10);

	/** @addtogroup CoreGroup */
	/** @{ */
	
	class WPO_WCLABELS_QRmask {
		...
		{
			$this->runLength = array_fill(0, WPO_WCLABELS_QRSPEC_WIDTH_MAX + 1, 0);
		}
		...
		{
			$blacks = 0;
			$format =  WPO_WCLABELS_QRspec::getFormatInfo($mask, $level);
		...
		public function makeMaskNo($maskNo, $width, $s, &$d, $maskGenOnly = false) 
		{
			...
			$fileName = WPO_WCLABELS_QR_CACHE_DIR.'mask_'.$maskNo.DIRECTORY_SEPARATOR.'mask_'.$width.'_'.$maskNo.'.dat';
			...
			if (WPO_WCLABELS_QR_CACHEABLE) {
				if (file_exists($fileName)) {
					...
					if (!file_exists(WPO_WCLABELS_QR_CACHE_DIR.'mask_'.$maskNo))
						mkdir(WPO_WCLABELS_QR_CACHE_DIR.'mask_'.$maskNo);
		...
		public function calcN1N3($length)
		{
			...
			for($i=0; $i<$length; $i++) {
				...
				if($this->runLength[$i] >= 5) {
					$demerit += (WPO_WCLABELS_N1 + ($this->runLength[$i] - 5));
				}
				if($i & 1) {
		...
		public function mask($width, $frame, $level)
		{
			...
			if (WPO_WCLABELS_QR_FIND_FROM_RANDOM !== false) {
			
				$howManuOut = 8-(WPO_WCLABELS_QR_FIND_FROM_RANDOM % 9);
			...
			foreach($checked_masks as $i) {
				...
				$demerit = (int)((int)(abs($blacks - 50) / 5) * WPO_WCLABELS_N4);
 ...
 */
	...
	define('WPO_WCLABELS_QR_AREA_N', 0);
	define('WPO_WCLABELS_QR_AREA_E', 1);
	define('WPO_WCLABELS_QR_AREA_S', 2);
	define('WPO_WCLABELS_QR_AREA_W', 3);
	
	define('WPO_WCLABELS_QR_AREA_X', 0);
	define('WPO_WCLABELS_QR_AREA_Y', 1);
	
	define('WPO_WCLABELS_QR_AREA_TRACKER', 0);
	define('WPO_WCLABELS_QR_AREA_PATH',    1);
	define('WPO_WCLABELS_QR_AREA_POINT',   2);
	define('WPO_WCLABELS_QR_AREA_RECT',    3);
	define('WPO_WCLABELS_QR_AREA_LSHAPE',  4);
	
	/** @addtogroup OutputGroup */
	/** @{ */
	
	class WPO_WCLABELS_QRareaGroup {
		public $total = 0;
	...
	class WPO_WCLABELS_QRarea {
		...
		public function __construct($source_tab) 
		{
			...
			$this->paths[] = array(WPO_WCLABELS_QR_AREA_TRACKER, 0,0);
			$this->paths[] = array(WPO_WCLABELS_QR_AREA_TRACKER, 0,($this->width-7));
			$this->paths[] = array(WPO_WCLABELS_QR_AREA_TRACKER, ($this->width-7),0);
		...
		public function dumpEdges() 
		{
			...
			foreach($this->tab_edges as $line) {
				...
				foreach($line as $item) {
					...
					$styles = 'border-top:';
					if ($item[WPO_WCLABELS_QR_AREA_N])
							$styles .=  $style_on;
					else    $styles .=  $style_off;
					
					$styles .= 'border-bottom:';
					if ($item[WPO_WCLABELS_QR_AREA_S])
							$styles .=  $style_on;
					else    $styles .=  $style_off;
					
					$styles .= 'border-right:';
					if ($item[WPO_WCLABELS_QR_AREA_E])
							$styles .=  $style_on;
					else    $styles .=  $style_off;
					
					$styles .= 'border-left:';
					if ($item[WPO_WCLABELS_QR_AREA_W])
							$styles .=  $style_on;
					else    $styles .=  $style_off;
				...
				for ($sx = 0; $sx < $this->width; $sx++) {
					...
					if ($this->tab[$sy][$sx] == 1) { // non-allocated
						...
						if (!$grouped) {
							...
							$this->groups[$this->curr_group] = new WPO_WCLABELS_QRareaGroup($this->curr_group, $sx, $sy);
						}
		...
		private function detectSquare($group)
		{
			...
			foreach($group->points as $elem) {
				$min_x = min($min_x, $elem[WPO_WCLABELS_QR_AREA_X]);
				$max_x = max($max_x, $elem[WPO_WCLABELS_QR_AREA_X]);
				$min_y = min($min_y, $elem[WPO_WCLABELS_QR_AREA_Y]);
				$max_y = max($max_y, $elem[WPO_WCLABELS_QR_AREA_Y]);
			}
		...
		public function detectAreas()
		{
			...
			foreach ($this->groups as $groupId=>&$group) {
				if ($group->total > 3) {
					...
					if ((!$group->vertical)||(!$group->horizontal)) {
						...
						array_unshift($squareCoord, WPO_WCLABELS_QR_AREA_RECT);
						...
						$this->paths[] = array(WPO_WCLABELS_QR_AREA_PATH, $group->paths);
					}
					...
					$lshapes[] = $squareCoord[WPO_WCLABELS_QR_AREA_X];
					$lshapes[] = $squareCoord[WPO_WCLABELS_QR_AREA_Y];
					$lshapes[] = $variant;
			...
			if (count($points) > 0) {
				array_unshift($points, WPO_WCLABELS_QR_AREA_POINT);
				$this->paths[] = $points;
			}
			
			if (count($squares) > 0) {
				array_unshift($squares, WPO_WCLABELS_QR_AREA_RECT);
				$this->paths[] = $squares;
			}
			
			if (count($lshapes) > 0) {
				array_unshift($lshapes, WPO_WCLABELS_QR_AREA_LSHAPE);
				$this->paths[] = $lshapes;
			}
		}
		...
		{
			$this->tab_edges[$elem[WPO_WCLABELS_QR_AREA_Y]][$elem[WPO_WCLABELS_QR_AREA_X]][$edgeNo] = true;
		}
		...
		private function markAdjacentEdges($group)
		{
			foreach($group->points as $elem) {
				if ($this->getOnElem($elem, -1, 0) == $group->id)
					$this->reserveEdgeOnElem($elem, WPO_WCLABELS_QR_AREA_W);
					
				if ($this->getOnElem($elem, +1, 0) == $group->id)
					$this->reserveEdgeOnElem($elem, WPO_WCLABELS_QR_AREA_E);
					
				if ($this->getOnElem($elem, 0, -1) == $group->id)
					$this->reserveEdgeOnElem($elem, WPO_WCLABELS_QR_AREA_N);
				
				if ($this->getOnElem($elem, 0, +1) == $group->id)
					$this->reserveEdgeOnElem($elem, WPO_WCLABELS_QR_AREA_S);
			}
		}
		...
		private function detectPaths(&$group)
		{
			...
			$elem = $group->points[0];
			$waylist = $this->findPath($group, $elem[WPO_WCLABELS_QR_AREA_X], $elem[WPO_WCLABELS_QR_AREA_Y]);
			$group->paths[] = array($elem[WPO_WCLABELS_QR_AREA_X], $elem[WPO_WCLABELS_QR_AREA_Y], $waylist);
			
			$tab = array();
			foreach($group->points as $elem) {
				
				$edgeTab = $this->tab_edges[$elem[WPO_WCLABELS_QR_AREA_Y]][$elem[WPO_WCLABELS_QR_AREA_X]];
				
				if (!(  $edgeTab[WPO_WCLABELS_QR_AREA_N] 
					&&  $edgeTab[WPO_WCLABELS_QR_AREA_E] 
					&&  $edgeTab[WPO_WCLABELS_QR_AREA_S] 
					&&  $edgeTab[WPO_WCLABELS_QR_AREA_W])) {
					
					if (!$edgeTab[WPO_WCLABELS_QR_AREA_S]) {
					   
						$waylistw = $this->findPath($group, $elem[WPO_WCLABELS_QR_AREA_X], $elem[WPO_WCLABELS_QR_AREA_Y]+1);
						$group->paths[] = array($elem[WPO_WCLABELS_QR_AREA_X], $elem[WPO_WCLABELS_QR_AREA_Y]+1, $waylistw);
					}
				}
			}
		...
		private function findPath($group, $sx, $sy)
		{
			...
			do
			{
				...
				while ((count($move_expl) > 0)&&($have_way == false)) {
					...
					if (($have_way==false)&&($way=='R')&&($this->tab_edges[$py][$px][WPO_WCLABELS_QR_AREA_N]==false)) {
						$have_way = true;
						$dir = $way;
						$this->reserveEdge($px, $py, WPO_WCLABELS_QR_AREA_N);
						$px++;                      
					} 
					
					if (($have_way==false)&&($way=='B')&&($this->tab_edges[$py][$px-1][WPO_WCLABELS_QR_AREA_E]==false)) {
						$have_way = true;
						$dir = $way;
						$this->reserveEdge($px-1, $py, WPO_WCLABELS_QR_AREA_E);
						$py++;                      
					} 
					
					if (($have_way==false)&&($way=='L')&&($this->tab_edges[$py-1][$px-1][WPO_WCLABELS_QR_AREA_S]==false)) {
						$have_way = true;
						$dir = $way;
						$this->reserveEdge($px-1, $py-1, WPO_WCLABELS_QR_AREA_S);
						$px--;                      
					} 
					
					if (($have_way==false)&&($way=='T')&&($this->tab_edges[$py-1][$px][WPO_WCLABELS_QR_AREA_W]==false)) {
						$have_way = true;
						$dir = $way;
						$this->reserveEdge($px, $py-1, WPO_WCLABELS_QR_AREA_W);
						$py--;                      
					} 
				}
 ...
 */
	...
	class WPO_WCLABELS_QRcanvasOutput extends WPO_WCLABELS_QRarea {
		...
		public function getCanvasOps()
		{
			...
			foreach ($this->paths as $path) {
				switch ($path[0]) {
					case WPO_WCLABELS_QR_AREA_PATH:
							$pNum = 0;
						...
						break;
					case WPO_WCLABELS_QR_AREA_POINT:
					...
					case WPO_WCLABELS_QR_AREA_RECT:
					...
					case WPO_WCLABELS_QR_AREA_LSHAPE:
					...
					case WPO_WCLABELS_QR_AREA_TRACKER:
 ...
 */
	...
	class WPO_WCLABELS_QRsvgOutput extends WPO_WCLABELS_QRarea {
 ...
 */
	...
	*/
	class WPO_WCLABELS_QRrsblock {
		...
		@param WPO_WCLABELS_QRrsItem $rs RS encoding item
		*/
		public function __construct($dl, $data, $el, &$ecc, WPO_WCLABELS_QRrsItem $rs)
		{
	...
	class WPO_WCLABELS_QRrawcode {
		...
		@param WPO_WCLABELS_QRinput $input input stream
		*/
		public function __construct(WPO_WCLABELS_QRinput $input)
		{
			$spec = array(0,0,0,0,0);
			...
			WPO_WCLABELS_QRspec::getEccSpec($input->getVersion(), $input->getErrorCorrectionLevel(), $spec);

			$this->version = $input->getVersion();
			$this->b1 = WPO_WCLABELS_QRspec::rsBlockNum1($spec);
			$this->dataLength = WPO_WCLABELS_QRspec::rsDataLength($spec);
			$this->eccLength = WPO_WCLABELS_QRspec::rsEccLength($spec);
			$this->ecccode = array_fill(0, $this->eccLength, 0);
			$this->blocks = WPO_WCLABELS_QRspec::rsBlockNum($spec);
		...
		@param Array $spec code speciffigation, as provided by WPO_WCLABELS_QRspec
		*/
		public function init(array $spec)
		{
			$dl = WPO_WCLABELS_QRspec::rsDataCodes1($spec);
			$el = WPO_WCLABELS_QRspec::rsEccCodes1($spec);
			$rs = WPO_WCLABELS_QRrs::init_rs(8, 0x11d, 0, 1, $el, 255 - $dl - $el);
			

			$blockNo = 0;
			$dataPos = 0;
			$eccPos = 0;
			for($i=0; $i<WPO_WCLABELS_QRspec::rsBlockNum1($spec); $i++) {
				$ecc = array_slice($this->ecccode,$eccPos);
				$this->rsblocks[$blockNo] = new WPO_WCLABELS_QRrsblock($dl, array_slice($this->datacode, $dataPos), $el,  $ecc, $rs);
			...
			if(WPO_WCLABELS_QRspec::rsBlockNum2($spec) == 0)
				return 0;

			$dl = WPO_WCLABELS_QRspec::rsDataCodes2($spec);
			$el = WPO_WCLABELS_QRspec::rsEccCodes2($spec);
			$rs = WPO_WCLABELS_QRrs::init_rs(8, 0x11d, 0, 1, $el, 255 - $dl - $el);
			
			if($rs == NULL) return -1;
			
			for($i=0; $i<WPO_WCLABELS_QRspec::rsBlockNum2($spec); $i++) {
				$ecc = array_slice($this->ecccode,$eccPos);
				$this->rsblocks[$blockNo] = new WPO_WCLABELS_QRrsblock($dl, array_slice($this->datacode, $dataPos), $el, $ecc, $rs);
				$this->ecccode = array_merge(array_slice($this->ecccode,0, $eccPos), $ecc);
	...
	//##########################################################################
	/** 
	__Main class WPO_WCLABELS_to create QR-code__.
	QR Code symbol is a 2D barcode that can be scanned by handy terminals such as a mobile phone with CCD.
	The capacity of QR Code is up to 7000 digits or 4000 characters, and has high robustness.
	This class WPO_WCLABELS_supports QR Code model 2, described in JIS (Japanese Industrial Standards) X0510:2004 or ISO/IEC 18004.
	...
	*/
	...
	class WPO_WCLABELS_QRcode {
	
		public $version;    ///< __Integer__ QR code version. Size of WPO_WCLABELS_QRcode is defined as version. Version is from 1 to 40. Version 1 is 21*21 matrix. And 4 modules increases whenever 1 version increases. So version 40 is 177*177 matrix.
		...
		If canvas js support library was included, we remember it static in WPO_WCLABELS_QRcode. 
		...
		@param WPO_WCLABELS_QRinput $input data object
		@param Integer $mask sugested masking mode
		@return WPO_WCLABELS_QRcode $this (current instance)
		*/
		public function encodeMask(WPO_WCLABELS_QRinput $input, $mask)
		{
			if($input->getVersion() < 0 || $input->getVersion() > WPO_WCLABELS_QRSPEC_VERSION_MAX) {
				throw new Exception('wrong version');
			}
			if($input->getErrorCorrectionLevel() > WPO_WCLABELS_QR_ECLEVEL_H) {
				throw new Exception('wrong level');
			}

			$raw = new WPO_WCLABELS_QRrawcode($input);
			
			WPO_WCLABELS_QRtools::markTime('after_raw');
			
			$version = $raw->version;
			$width = WPO_WCLABELS_QRspec::getWidth($version);
			$frame = WPO_WCLABELS_QRspec::newFrame($version);
			
			$filler = new WPO_WCLABELS_QRframeFiller($width, $frame);
			if(is_null($filler)) {
				return NULL;
			}
			...
			WPO_WCLABELS_QRtools::markTime('after_filler');
			
			unset($raw);
			
			// remainder bits
			$j = WPO_WCLABELS_QRspec::getRemainder($version);
			for($i=0; $i<$j; $i++) {
				$addr = $filler->next();
			...
			// masking
			$maskObj = new WPO_WCLABELS_QRmask();
			if($mask < 0) {
			
				if (WPO_WCLABELS_QR_FIND_BEST_MASK) {
					$masked = $maskObj->mask($width, $frame, $input->getErrorCorrectionLevel());
				} else {
					$masked = $maskObj->makeMask($width, $frame, (intval(WPO_WCLABELS_QR_DEFAULT_MASK) % 8), $input->getErrorCorrectionLevel());
				}
			} else {
			...
			WPO_WCLABELS_QRtools::markTime('after_mask');
		...
		@param WPO_WCLABELS_QRinput $input data object to be encoded
		@return 
		*/
		public function encodeInput(WPO_WCLABELS_QRinput $input)
		{
		...
		*/
		public function encodeString8bit($string, $version, $level)
		{
			...
			$input = new WPO_WCLABELS_QRinput($version, $level);
			if($input == NULL) return NULL;

			$ret = $input->append(WPO_WCLABELS_QR_MODE_8, strlen($string), str_split($string));
			if($ret < 0) {
				unset($input);
				return NULL;
			}
		...
		if not - encoder may use optimal WPO_WCLABELS_QR_MODE_AN instead of WPO_WCLABELS_QR_MODE_8
		@return WPO_WCLABELS_QRcode $this (current instance)
		*/
		public function encodeString($string, $version, $level, $hint, $casesensitive)
		{

			if($hint != WPO_WCLABELS_QR_MODE_8 && $hint != WPO_WCLABELS_QR_MODE_KANJI) {
			...
			$ret = WPO_WCLABELS_QRsplit::splitStringToQRinput($string, $input, $hint, $casesensitive);
			if($ret < 0) {
				return NULL;
			}
		...
		*/
		...
		public static function png($text, $outfile = false, $level = WPO_WCLABELS_QR_ECLEVEL_L, $size = 3, $margin = 4, $saveandprint=false) 
		{
			$enc = WPO_WCLABELS_QRencode::factory($level, $size, $margin);
		...
		*/
		...
		public static function text($text, $outfile = false, $level = WPO_WCLABELS_QR_ECLEVEL_L, $size = 3, $margin = 4) 
		{
			$enc = WPO_WCLABELS_QRencode::factory($level, $size, $margin);
		...
		*/
		...
		public static function raw($text, $outfile = false, $level = WPO_WCLABELS_QR_ECLEVEL_L, $size = 3, $margin = 4) 
		{
			$enc = WPO_WCLABELS_QRencode::factory($level, $size, $margin);
		...
		*/
		...
		public static function canvas($text, $elemId = false, $level = WPO_WCLABELS_QR_ECLEVEL_L, $width = false, $size = false, $margin = 4, $autoInclude = false) 
		{
			$html = '';
			$extra = '';
			...
			$enc = WPO_WCLABELS_QRencode::factory($level, 1, 0);
			$tab_src = $enc->encode($text, false);
			$area = new WPO_WCLABELS_QRcanvasOutput($tab_src);
			$area->detectGroups();
			$area->detectAreas();
		...
		*/
		...
		public static function svg($text, $elemId = false, $outFile = false, $level = WPO_WCLABELS_QR_ECLEVEL_L, $width = false, $size = false, $margin = 4, $compress = false) 
		{
			$enc = WPO_WCLABELS_QRencode::factory($level, 1, 0);
			$tab_src = $enc->encode($text, false);
			$area = new WPO_WCLABELS_QRsvgOutput($tab_src);
			$area->detectGroups();
			$area->detectAreas();
	...
	*/
	class WPO_WCLABELS_QRframeFiller {
	...
	class WPO_WCLABELS_QRencode {
		...
		public $level = WPO_WCLABELS_QR_ECLEVEL_L; ///< __Integer__ error correction level __QR_ECLEVEL_L__, __QR_ECLEVEL_M__, __QR_ECLEVEL_Q__ or __QR_ECLEVEL_H__
		public $hint = WPO_WCLABELS_QR_MODE_8;     ///< __Integer__ encoding hint, __QR_MODE_8__ or __QR_MODE_KANJI__, Because Kanji encoding is kind of 8 bit encoding we need to hint encoder to use Kanji mode explicite. (otherwise it may try to encode it as plain 8 bit stream)
		...
		@return builded WPO_WCLABELS_QRencode instance
		*/
		public static function factory($level = WPO_WCLABELS_QR_ECLEVEL_L, $size = 3, $margin = 4)
		{
			$enc = new WPO_WCLABELS_QRencode();
			$enc->size = $size;
			$enc->margin = $margin;
			...
			switch ($level.'') {
				...
				case 'l':
				case 'L':
						$enc->level = WPO_WCLABELS_QR_ECLEVEL_L;
					break;
				case 'm':
				case 'M':
						$enc->level = WPO_WCLABELS_QR_ECLEVEL_M;
					break;
				case 'q':
				case 'Q':
						$enc->level = WPO_WCLABELS_QR_ECLEVEL_Q;
					break;
				case 'h':
				case 'H':
						$enc->level = WPO_WCLABELS_QR_ECLEVEL_H;
					break;
			}
		...
		*/
		public function encode($intext, $outfile = false) 
		{
			...
			WPO_WCLABELS_QRtools::markTime('after_encode');
			
			$binarized = WPO_WCLABELS_QRtools::binarize($code->data);
			if ($outfile!== false) {
		...
		*/
		public function encodePNG($intext, $outfile = false, $saveandprint=false) 
		{
			try {
				...
				if ($err != '')
					WPO_WCLABELS_QRtools::log($outfile, $err);
				
				$maxSize = (int)(WPO_WCLABELS_QR_PNG_MAXIMUM_SIZE / (count($tab)+2*$this->margin));
				
				WPO_WCLABELS_QRimage::png($tab, $outfile, min(max(1, $this->size), $maxSize), $this->margin,$saveandprint);
			
			} catch (Exception $e) {
			
				WPO_WCLABELS_QRtools::log($outfile, $e->getMessage());
