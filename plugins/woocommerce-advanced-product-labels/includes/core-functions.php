...
function wapl_get_label_html( $args ) {
	...
	// WPML
	$wpml_package = array(
		'kind' => 'Advanced Product Label',
		'name' => $label['id'],
		'title' => sprintf( '"%s" product label text', $label['post_title'] ?? '' ),
	);
	$label['text'] = apply_filters( 'wpml_translate_string', $label['text'] , 'text', $wpml_package );
	...
	ob_start();
		...
		?><style class="wapl-custom-css"><?php echo $label['enable_advanced'] ? preg_replace('/\s+/', '', $label['custom_css'] ) : ''; ?></style><?php
...
/**
 * Global label string translate compatibility.
 *
 * @param  int  $id   Label ID being saved.
 * @param array $data Data stored.
 */
function wapl_wpml_string_translate( $id, $data ) {
	$wpml_package = array(
		'kind' => 'Advanced Product Label',
		'name' => $id,
		'title' => sprintf( '"%s" product label text', $label['post_title'] ?? '' ),
	...
	do_action( 'wpml_register_string', $data['text'], 'text', $wpml_package, 'Text', 'LINE' );
}
add_action( 'wapl_save_label', 'wapl_wpml_string_translate', 10, 2 );

/**
 * Product label WPML support.
 *
 * Add support for string translation for product labels.
 *
 * @param  string  $type Product type.
 * @param  WP_Post $post Post object,
 */
function wapl_single_label_wpml_string_translate( $type, $post ) {
    $product = wc_get_product( $post );

	$wpml_package = array(
		'kind' => 'Advanced Product Label',
		'name' => $product->get_id(),
		'title' => sprintf( '"%s" product label text', $product->get_title() ),
	);

	do_action( 'wpml_register_string', sanitize_text_field( $_POST['_wapl_label_text'] ), 'text', $wpml_package, 'Text', 'LINE' );
}
add_action( 'woocommerce_process_product_meta', 'wapl_single_label_wpml_string_translate', 10, 2 );
