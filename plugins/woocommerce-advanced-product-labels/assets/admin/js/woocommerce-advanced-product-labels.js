jQuery( function( $ ) {
	...
	function onAlignUpdate(e) {
		...
		labelWrap.classList.remove('wapl-alignnone', 'wapl-alignleft', 'wapl-alignright', 'wapl-aligncenter', 'wapl-aligncustom');
	...
	document.body.addEventListener('keydown', (e) => {
		if (alignField.value !== 'custom') return;

		var activeElement = document.activeElement,
			inputs = ['input', 'select', 'button', 'textarea', 'div'],
			distance = e.shiftKey ? 10 : 1,
			left = label.style.left || label.offsetLeft,
			top = label.style.top || label.offsetTop;

		// Stop if focussed in another field
...
// 		palettes: ['#D9534F', '#3498db', '#39A539', '#ffe312', '#ffA608', '#999', '#444', '#fff'],
	...
	// Advanced toggle
	$(document.body).on('click', '.wapl-toggle-advanced', function(e) {
		if ($(this).hasClass('woocommerce-input-toggle--enabled')) {
			$(this).removeClass('woocommerce-input-toggle--enabled').addClass('woocommerce-input-toggle--disabled');
			document.querySelector('[name="_wapl_label[enable_advanced]"]').checked = false;
			$(this).parents('.wapl-global-option').removeClass('advanced-enabled')
			document.querySelector("#wapl-custom-css").innerHTML = ''; // Clear style tag
		} else {
			$(this).removeClass('woocommerce-input-toggle--disabled').addClass('woocommerce-input-toggle--enabled');
			document.querySelector('[name="_wapl_label[enable_advanced]"]').checked = true;
			$(this).parents('.wapl-global-option').addClass('advanced-enabled')
			window.wapl.codeEditor.codemirror.refresh(); // Refresh needed to load CM properly :/
			document.querySelector("#wapl-custom-css").innerHTML = window.wapl.codeEditor.codemirror.getValue().trim(); // Populate style tag
		}
	});

	// Init code editor
	window.wapl.codeEditor = wp.codeEditor.initialize( "_wapl_label_custom_css", wapl.codeEditorSettings );
		window.wapl.codeEditor.codemirror.on("change", function (cm, changeObj) {
		document.querySelector("#wapl-custom-css").innerHTML = cm.getValue().trim();
	});
});
