...
if (!class_exists('WC_SA_Email')) :
    ...
    class WC_SA_Email extends WC_Email
    {
        ...
        public function __construct($st_id)
        {
            ...
            $this->title = sprintf(__('%s', 'woocommerce_status_actions'), $status->title);
            $this->description = sprintf(__("This is an order notification sent to customers when their order status is marked %s", 'woocommerce_status_actions'), $status->title);

            $this->heading = stripslashes($status->email_heading);
            $this->subject = stripslashes($status->email_subject);
            $this->attachments = $status->get_attachments();
...
//            $this->placeholders['{order_date}'] = date_i18n(wc_date_format(), strtotime($this->object->order_date));
            ...
            $this->replace['order_date'] = date_i18n(wc_date_format(), strtotime($this->object->get_date_created()));
...
//            $this->placeholders['{order_value}'] = $this->object->order_total;
            $this->find['order_value'] = '{order_value}';
            $this->replace['order_value'] = $this->object->get_formatted_order_total();

//            $this->placeholders['{payment_method}'] = $this->object->payment_method_title;
            $this->find['payment_method'] = '{payment_method}';
            $this->replace['payment_method'] = $this->object->get_payment_method_title();

//            $this->placeholders['{shipping_method}'] = $this->object->get_shipping_method();
            $this->find['shipping_method'] = '{shipping_method}';
            $this->replace['shipping_method'] = $this->object->get_shipping_method();

//            $this->placeholders['{billing_address}'] = $this->object->get_formatted_billing_address();
            $this->find['billing_address'] = '{billing_address}';
            $this->replace['billing_address'] = $this->object->get_formatted_billing_address();

//            $this->placeholders['{shipping_address}'] = $this->object->get_formatted_shipping_address();
            $this->find['shipping_address'] = '{shipping_address}';
            $this->replace['shipping_address'] = $this->object->get_formatted_shipping_address();

            $customer_first_name = "";
            $customer_last_name = "";
            $customer_user_object = get_user_by('id', $this->object->get_customer_id());
...
//            $this->placeholders['{customer_first_name}'] = stripslashes($customer_first_name);
//            $this->placeholders['{customer_last_name}'] = stripslashes($customer_last_name);
